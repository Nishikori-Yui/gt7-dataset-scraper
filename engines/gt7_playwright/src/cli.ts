...
async function getThumbMapFromPage(page: Page, locale: string, timeoutMs: number): Promise<Map<string, string[]>> {
  ...
  const rawMap = await page.evaluate(() => {
    ...
    const addThumb = (
      ...
      requireThumbPath: boolean,
    ): void => {
      const value = (rawUrl || "").trim();
      if (!value) return;
      if (requireThumbPath && !value.includes("/car_thumbnails/")) return;
    ...
    const styleUrl = (styleText: string | null | undefined): string => {
      const style = (styleText || "").trim();
      if (!style) return "";
      const match = style.match(/url\(["']?([^"')]+)["']?\)/i);
      return match?.[1] || "";
    };
    const parseSrcSet = (srcsetValue: string | null | undefined): string[] => {
    ...
    for (const img of Array.from(document.querySelectorAll("img"))) {
      ...
      for (const item of candidates) addThumb("", item, true);
      for (const item of parseSrcSet(img.getAttribute("srcset"))) addThumb("", item, true);
      for (const item of parseSrcSet(img.getAttribute("data-srcset"))) addThumb("", item, true);
    }
    ...
    for (const anchor of Array.from(document.querySelectorAll("a[href*='/gt7/carlist/id/']"))) {
      ...
      for (const node of imgNodes) {
        ...
        for (const item of candidates) addThumb(carId, item, false);
        for (const item of parseSrcSet(node.getAttribute("srcset"))) addThumb(carId, item, false);
        for (const item of parseSrcSet(node.getAttribute("data-srcset"))) addThumb(carId, item, false);
      }
  ...
  for (const [carId, thumbs] of Object.entries(rawMap)) {
    if (!Array.isArray(thumbs) || thumbs.length === 0) continue;
    result.set(carId, thumbs);
  }
  return result;
}

async function extractDetail(page: Page, locale: string, carId: string, timeoutMs: number): Promise<Record<string, unknown>> {
...
async function readCarIdsFromStdin(): Promise<string[]> {
  const ids: string[] = [];
  ...
  for await (const line of rl) {
    ...
    if (!text) continue;
    try {
      const obj = JSON.parse(text) as { car_id?: string };
      if (obj.car_id && obj.car_id.trim().length > 0) ids.push(obj.car_id.trim());
    } catch {}
  }
  return ids;
}
